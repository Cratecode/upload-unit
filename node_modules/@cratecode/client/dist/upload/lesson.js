 ...
 */
function handleLesson(state, id, name, description, unit, spec, extendsTemplate, lessonClass, dir) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const entry of walkEntries) {
            ...
            if (entry[0].endsWith("/README.md")) {
                files[entry[2]] = files[entry[2]].replace(/\$\$IMAGE\s+([^$\s]+)(?:\s+([^$]+))?\$\$/g, (_, name, alt) => {
                    if (!state.images) {
                        throw new Error("An images folder must be set before images can be used.");
                    }
                    const fileRecord = state.images[name];
                    if (!fileRecord) {
                        throw new Error('Could not find image "' + name + '".');
                    }
                    const sizeMetadata = fileRecord.width && fileRecord.height
                        ? "{" +
                            fileRecord.width +
                            "x" +
                            fileRecord.height +
            ...
            switch (message.type) {
                ...
                case ProxyOut.MessageType.Files: {
                    ...
                    for (const file of inner_message.files) {
                        ...
                        else if (files[fixedPath] === ((_a = file.data) === null || _a === void 0 ? void 0 : _a.toString())) {
